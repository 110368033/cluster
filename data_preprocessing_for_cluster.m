function [dist_mid,ssim_mid,dist_all,ssim_all,data_smooth]=data_preprocessing_for_cluster(data)
...
%     plot(data(:,i));title('raw data');
    data_nor=normalize(data(:,i), 'range', [0 1]);
...
fft_result1=fft_process_for_cluster(data_smooth); 
fs=100;
...
fft_result2=fft_process_for_cluster(data2);
N=size(data2,1);
t = -(N-1)/fs:1/fs:(N-1)/fs;
data3=[];
...
fft_result3=fft_process_for_cluster(data3);
N=size(data3,1);
t = -(N-1)/fs:1/fs:(N-1)/fs;
